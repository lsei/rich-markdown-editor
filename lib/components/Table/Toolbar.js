...
var Toolbar = function (_React$Component) {
  ...
  _createClass(Toolbar, [{
    ...
    value: function getDerivedStateFromProps(nextProps, prevState) {
      ...
      var element = (0, _reactDom.findDOMNode)(nextProps.cell);
      if (!(element instanceof HTMLElement)) return;

      var rect = element.getBoundingClientRect();
      var menuWidth = MENU_WIDTHS[nextProps.type];
      var menuHeight = 40;

      // Position the menu correctly depending on the type, cell and scroll position
      var left = MENU_ALIGN[nextProps.type] === "center" ? Math.round(rect.left + window.scrollX + rect.width / 2 - menuWidth / 2) : Math.round(rect.left + window.scrollX - menuWidth / 2);
